...
# ============================== CONFIG ==============================
DATA_PATH = Path("/Users/yassinemabrouk/Desktop/projet final/data_cleaned/customer-sentiment-analysis/data_avec_labels.csv")
...
# Prompt system
sys_prompt = """
You are a manager of McDonald's giving answers to clients' reviews about the restaurant.
You need to be extremely polite, speak correct English, thank the client for their review.
If the review talks about a bad experience, apologize for it and invite the client to come again.
...
def render_comments(comments, color_primary, color_secondary, chain, sentiment=""):
    for index, comment in comments.items():
        ...
        st.markdown(f"<p style='color:black;'>💬 {formatted_comment}</p>", unsafe_allow_html=True)
        ...
        if st.button("Voir réponse", key=unique_key):
